...
export WORKSPACE=~/Code/scv-labs/avi-on-dvp
export CURRENT_FILENAME=$(basename -- "$0")
# Internal variables
export POC_WORKSPACE=${WORKSPACE}/poc
...
function do_tmux_session() {
  cd $WORKSPACE
  tmux new -s 'DVP POC' -n 'LunarVim'\; send-keys " lvim" ENTER \
...
function do_start() {
  ...
  createCommandPane "Backend" \
    ${POC_WORKSPACE} \
    "sleep 5; ${DOCKER_CALL} up -d; ${DOCKER_CALL} logs -f --tail 100 server"
...
function do_api_token() {
  curl -sS --location 'https://api-qa.avi-on.com/sessions' \
    ...
    }' | tee >(jq '.') |jq -r '.credentials.auth_token' | pbcopy
}
